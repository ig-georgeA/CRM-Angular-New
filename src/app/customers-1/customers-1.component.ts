...
import { IGridEditDoneEventArgs, IGX_DIALOG_DIRECTIVES, IGX_GRID_ACTION_STRIP_DIRECTIVES, IGX_GRID_DIRECTIVES, IGX_INPUT_GROUP_DIRECTIVES, IGX_SELECT_DIRECTIVES, IgxButtonDirective, IgxDialogComponent, IgxIconComponent, IgxPaginatorComponent, IgxSnackbarComponent, IRowDataEventArgs } from 'igniteui-angular';
...
@Component({
  selector: 'app-customers-1',
  imports: [IGX_GRID_ACTION_STRIP_DIRECTIVES, IGX_INPUT_GROUP_DIRECTIVES, IGX_SELECT_DIRECTIVES, IGX_DIALOG_DIRECTIVES, IGX_GRID_DIRECTIVES, IgxIconComponent, IgxButtonDirective, IgxPaginatorComponent, IgxSnackbarComponent, ReactiveFormsModule, FormsModule],
  templateUrl: './customers-1.component.html',
  styleUrls: ['./customers-1.component.scss']
})
export class Customers1Component implements OnInit, OnDestroy {
  ...
  public grid_Data_Request1?: CustomerDtoPagedResultDto;
  public grid_Data_Request1$: Subject<void> = new Subject<void>();


  private _grid_Page_Index1: number = 0;
  public get grid_Page_Index1(): number {
    return this._grid_Page_Index1;
  }
  public set grid_Page_Index1(value: number) {
    this._grid_Page_Index1 = value;
    this.grid_Data_Request1$.next();
  }

  private _grid_Page_Size1: number = 20;
  public get grid_Page_Size1(): number {
    return this._grid_Page_Size1;
  }
  public set grid_Page_Size1(value: number) {
    this._grid_Page_Size1 = value;
    this.grid_Data_Request1$.next();
  }
  public selectedCustomer2?: CustomerDto;
  public value?: string;
  ...
  ngOnInit() {
    this.northwindSwaggerService.getCustomerDtoPagedResultDto(this.grid_Page_Index1 ?? 0, this.grid_Page_Size1 ?? 0).pipe(takeUntil(this.destroy$)).subscribe(
      data => this.grid_Data_Request1 = data
    );
    this.grid_Data_Request1$.pipe(takeUntil(this.destroy$)).subscribe(() => {
      this.northwindSwaggerService.getCustomerDtoPagedResultDto(this.grid_Page_Index1 ?? 0, this.grid_Page_Size1 ?? 0).pipe(take(1)).subscribe(
        data => this.grid_Data_Request1 = data
      );
    });
  }
  ...
  ngOnDestroy() {
    ...
    this.grid_Data_Request1$.complete();
  }
  ...
  public async ngSubmitCustomerDto(): Promise<void> {
    ...
    if (data != null) {
      this.grid_Page_Index1 = parseFloat('0');
